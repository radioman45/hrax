# -*- coding: utf-8 -*-
"""
팀장 피드백 리포트 배치 생성 스크립트 (L002~L006)
feedback-report-writer 스킬 규칙에 따라 DOCX 2페이지 생성
헤더/섹션 제목: #000000 (검정), 배경: #F5F5F5 (SK Orange 사용 금지)
"""
...
def build_report(input_path: str, output_path: str):
  ...
  if issues:
    print(f"[WARNING] 민감정보 이슈 발견 ({input_path}): {issues}")
    return False
...
def main():
  ...
  for leader_id, leader_name in TARGETS:
    input_path = os.path.join(BASE_DIR, "_workspace", "01_data", f"{leader_id}_data.json")
    output_path = os.path.join(
      BASE_DIR, "survey-feedback", "output",
      f"leadership_feedback_{leader_id}_{leader_name}.docx"
    )
    ok = build_report(input_path, output_path)
    if ok:
  ...
  for g in generated:
    print(f" - {g}")
  print(f"총 {len(generated)}개 생성 완료.")
